...
Start() {
    ...
    if systemctl is-active --quiet mihomo; then
        echo -e "${Green_font_prefix}mihomo 正在运行${Font_color_suffix}"
        Start_Main
    fi
...
Restart() {
    ...
    if systemctl is-active --quiet mihomo; then
        ...
        echo -e "${Red_font_prefix}mihomo 重启失败，服务未激活${Font_color_suffix}"
        exit 1
    fi
    Start_Main
}
...
Install() {
    ...
    # 开始下载
    echo -e "${Green_font_prefix}开始下载 mihomo${Font_color_suffix}"
    ...
    # 
    if [ -f "$FILENAME" ]; then
        echo -e "${Green_font_prefix}下载完成, 开始部署${Font_color_suffix}"
        # 解压 
